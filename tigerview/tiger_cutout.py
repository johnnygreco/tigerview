...
__all__ = ['make_cutout', 'make_rgb_image']


def make_cutout(ra, dec, radius, band='i', skymap=None, butler=None, 
                pixscale=0.168, rerun=DEFAULT_RERUN, 
    ...
    if butler is None:
        butler = lsst.daf.persistence.Butler(rerun)
    if skymap is None:
    ...
    patches, _ = tracts_n_patches(radec_list, skymap=skymap, data_dir=rerun)
    ...
    stamp.setPsf(get_psf(largest_cutout))
    
    return stamp


def make_rgb_image(ra, dec, radius, butler=None, skymap=None, 
                   rgb='irg', Q=8, dataRange=0.6, rerun=DEFAULT_RERUN, 
                   img_size=None, return_wcs=False, 
                   coadd_label='deepCoadd_calexp'):
